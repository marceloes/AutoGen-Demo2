...
class BaseGroupChatAgent(RoutedAgent):
    ...
    async def handle_request_to_speak(self, message: RequestToSpeak, ctx: MessageContext) -> None:
        ...
        self._chat_history.append(
            UserMessage(content=f"Transferred to {self.id.type}, adopt the persona now.", source="system")
        )
        Console().print(f"Transferred to {self.id.type}, adopt the persona now.")
...
class IllustratorAgent(BaseGroupChatAgent):
    ...
    async def handle_request_to_speak(self, message: RequestToSpeak, ctx: MessageContext) -> None:  # type: ignore
        ...
        for tool_call in completion.content:
            ...
            image_data = self._image_gen_tool.return_value_as_string(result)
            image = Image.from_base64(image_data)
            #image = Image.from_pil(image.image.resize((256, 256)))
            #display(image.image)  # type: ignore
            pil_image = PILImage.open(io.BytesIO(base64.b64decode(image_data)))
            pil_image.show()
            images.append(image)
        await self.publish_message(
...
async def main() -> None:
    ...
    gpt_azure_deployment="gpt-4o-mini"
    gpt_model="gpt-4o-mini"
    gpt_api_version="2025-01-01-preview"
    gpt_azure_endpoint=os.getenv("AZURE_OPENAI_ENDPOINT")
    gpt_api_key=os.getenv("OPENAI_API_KEY")

    dalle3_azure_endpoint=os.getenv("DALLE3_ENDPOINT")
    dalle3_api_key=os.getenv("DALLE3_API_KEY")
    ...
    editor_agent_type = await EditorAgent.register(
        ...
        lambda: EditorAgent(
            ...
            model_client=AzureOpenAIChatCompletionClient(
                azure_deployment=gpt_azure_deployment,
                model=gpt_model,
    ...
    illustrator_agent_type = await IllustratorAgent.register(
        ...
        lambda: IllustratorAgent(
            ...
            image_client=AsyncAzureOpenAI(
                api_key=dalle3_api_key,
                azure_endpoint=dalle3_azure_endpoint,
                api_version="2024-02-01"                
            )
            # image_client=openai.AsyncClient(
            #     api_key=dalle3_api_key,
            #     base_url=dalle3_azure_endpoint
            # ),
        ),
    )
    ...
    await runtime.publish_message(
        GroupChatMessage(
            body=UserMessage(
                content="Please write a short story about the gingerbread man with up to 1 photo-realistic illustrations.",
                source="User",
            )
        ),
